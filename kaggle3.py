...
sample = pd.read_csv("D:/study/data/sample_submission.csv")
...
for train_index, test_index in skf.split(train, train.target.values):
    ...
    model.fit(x_train, utils.to_categorical(y_train), validation_data=(x_test, utils.to_categorical(y_test)), 
    ...
    submission = pd.DataFrame.from_dict({'id': test_ids, 'target': test_preds})
    submission.to_csv("submission.csv", index=False)
    
    #Epoch 00007: early stopping
    #0.7829501081478343
    #overall AUC=0.5018746539212203
